...
int tsdemux(LPVOID lpParam,int argc,char** argv)
{
    ...
    if(!ts::is_ts_filename(output))
    {
        if(join)
        {
            if(playlist.size())
            {
                ...
                ts::demuxer demuxer;
				//设置句柄，用于输出--------------
				demuxer.dlg=(CSpecialFTSDlg *)lpParam;
				//--------------
                int cn=1;
        ...
        }else
        {
            for(std::list<std::string>::iterator i=playlist.begin();i!=playlist.end();++i)
            {
				...
				demuxer.dlg=(CSpecialFTSDlg *)lpParam;
				//输出文件信息---------
				if(demuxer.dlg->m_ftsoutput==0){
					switch(demuxer.dlg->m_ftsdumpmode.GetCurSel()){
						case 0:demuxer.fp_dump=fopen("M2TS_timecodes.txt","wb+");break;
						case 1:demuxer.fp_dump=fopen("PTS_DTS.txt","wb+");break;
						case 2:demuxer.fp_dump=fopen("readable_PTS_DTS.txt","wb+");break;
					default:demuxer.fp_dump=fopen("M2TS_timecodes.txt","wb+");break;
					}
				}
				//------------------
...
*/
    ...
    fprintf(stderr,"\ntime: %li sec\n",time(0)-beg_time);
	CString finishtext;
	finishtext.LoadString(IDS_MSG_FINISH);
	AfxMessageBox(finishtext);
	//注意：一定要置0，否则只能运行一次！---------------
	//该项在运行结束后，不会自动归零
	optind=0;
	//---------------
