...
int ts::demuxer::demux_ts_packet(const char* ptr)
{
	...
	//试试
	//ts::demuxer::dlg->AppendTSLInfo(pid,0,0);
    ...
    if(!pid || (s.channel!=0xffff && s.type==0xff))
    {
        ...
        if(!pid)
        {
            // PAT
			// 输出
			ts::demuxer::dlg->AppendTSLInfo(pid,1,1);
            ptr+=5;
            ...
            for(int i=0;i<n;i++,ptr+=4)
            {
                ...
                pid&=0x1fff;
				//输出（只解析一次）
				if(ts::demuxer::parse_pat_ft==1){
					char temp_str1[100],temp_str2[100];
					//多国语言支持
					CStringA text;
					text.LoadString(IDS_TS_PROGRAM_PID);
					sprintf(temp_str1,text.GetBuffer(),i);
					itoa(pid,temp_str2,10);
					ts::demuxer::dlg->AppendBInfo("PAT",temp_str1,temp_str2,temp_str1);
				}
			...
			//如果解析过一次，就不再重复输出解析结果了了---------------
			ts::demuxer::parse_pat_ft=0;
        }else
        {
            // PMT
			// 输出
			ts::demuxer::dlg->AppendTSLInfo(pid,1,2);
            ptr+=7;
            ...
            while(ptr<end_ptr)
            {
                ...
                pid&=0x1fff;
				//输出（只解析一次）
				if(ts::demuxer::parse_pmt_ft==1){
					char temp_str1[100],temp_str2[100];
					//多国语言支持
					CStringA text;
					text.LoadString(IDS_TS_STREAM_PID);
					sprintf(temp_str1,text.GetBuffer(),type);
					itoa(pid,temp_str2,10);
					ts::demuxer::dlg->AppendBInfo("PMT",temp_str1,temp_str2,temp_str1);
				}
